...
const projects = [
  {
    ...
    description: 'Cấu hình giao diện landing page động thông qua hệ thống CMS quản trị',
  ...
  },
  {
    ...
    description: 'Xây dựng hệ thống lưu trữ và quản lý artifact (pull/push) cho developer và DevOps',
  ...
  },
  {
    ...
    description: 'Xây dựng hệ thống deployment tự động hỗ trợ developer và DevOps',
  ...
  },
  {
    ...
    description: 'Phát triển hệ thống cho phép chạy function tương tự AWS Lambda',
  ...
  },
  {
    ...
    description: 'Xây dựng website bán SIM, gói data và thiết bị viễn thông, tích hợp thanh toán ví điện tử',
  ...
  },
  {
    ...
    description: 'Xây dựng module nhận diện khuôn mặt cho nhân viên trong công ty',
...
export default function Projects() {
  ...
  // Xác định số card hiển thị theo kích thước màn hình
  React.useEffect(() => {
    const updateItems = () => {
  ...
  return (
    <section id="projects" className="relative bg-linear-to-b from-white via-blue-50 to-white">
      <div className="py-20">
        <div className="absolute top-0 right-0 w-96 h-96 bg-cyan-100 rounded-full blur-3xl opacity-20 -z-10"></div>
        <div className="max-w-7xl mx-auto relative">
          {/* Header */}
          <div className="text-center">
            <h3 className="text-xl text-center text-primary pb-4">Dự án đã thực hiện</h3>
            <h2 className="text-4xl font-bold text-primary">Một số dự án tiêu biểu</h2>
          ...
          <div className="relative">
                  ...
                  >
                    <div className="rounded-2xl cursor-pointer transition-all p-4 h-full group shadow-custom custom-card-shadow custom-card-shadow-hover">
                      ...
                      <h3 className="text-2xl font-bold text-primary my-4">
                        <Link href={`#`} className=" group-hover:text-cyan-400 transition-all duration-300">
                          {project.title}
                        </Link>
                      </h3>
                      ...
                      <div className="flex flex-wrap gap-2 pb-4">
                        {project.tags.map((category: string) => (
                          <span key={category} className="text-sm">{category}</span>
                        ))}
                      </div>
            ...
            {currentIndex > 0 && (
              <button
                ...
                className={`absolute lg:left-[15px] left-[30px] top-1/2 -translate-y-1/2 -translate-x-6 z-10 text-white rounded-full p-3 custom-btn-bg-gradient ${isAnimating ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer'
                  }`}
              >
                <ChevronLeft size={20} />
              </button>
            ...
            {currentIndex < maxIndex && (
              <button
                onClick={nextSlide}
                disabled={isAnimating}
                className={`absolute lg:right-[15px] right-[30px] top-1/2 -translate-y-1/2 translate-x-6 z-10 text-white rounded-full p-3 custom-btn-bg-gradient ${isAnimating ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer'
                  }`}
              >
